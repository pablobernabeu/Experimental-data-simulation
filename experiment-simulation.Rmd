---
title: Simulation
author: '<a href="https://github.com/pablobernabeu/Experiment-simulation-app" target="_blank"><button class="button-version">v 0.1</button></a> &nbsp; <a href = "#section-size-n"><button class="button-samplesize"><span style = "color: #FCFF89;"><em>N</em> = `r reactive(try(paste0(final_N()), silent = TRUE) %>% gsub("\\Error.*", "", .))`</span></button></a>'
output:
...
content: 'Please resume or revise parameters.'; 
...
/* Format the 'Replace this column with names/categories' field */
#section-adjust-sample-size > div.chart-stage > div > div:nth-child(8) > label {
max-width: 250%;
}
...
<i class="fas fa-info-circle" aria-hidden="true"></i>&nbsp; Home {style="background-color: #FCFCFC; data-width: 100%; width: 900px; margin: 0 auto;"}
=======================================================================

Column
-----------------------------------------------------------------------

### <span style='font-size:18px; color:#4D4F54;'> &nbsp; &nbsp; &nbsp; &nbsp;  Generate a pseudo-random data set to simulate an experiment.</span>

<div style = "padding-left: 60px; padding-right: 60px; background-color: #FEFEFE; font-size: 16px; line-height: 1.6; padding-top: 5px;">
...
A seed number is a code that allows the reproducibility of any data involving random generation. Please find your current seed number below, and feel free to note it down, or change it to any integer number up to Â±`r .Machine$integer.max`.
...
numericInput('seed', NULL, sample(1:99999, 1), min = 1, step = 1, max = .Machine$integer.max)
...
Reproducing a simulation requires three pieces of data: the seed number, the parameter list and the final data set. Therefore, after creating the variables wanted, make sure to download the parameter list and the final data set in the [last tab](#dataset).

### How to use

Independent variables are categorical and include between-group factors (distributing participants into different groups) and within-participant factors (administering a condition to all participants). Dependent variables are continuous.

<h3 style='color:#2C6367'> Current sample size (*N*): `r reactive(final_N())` ([adjust](#size-n)) </h3>
...
Categorical dependent variables are created in a binomial fashion ([`rbinom`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Binomial.html){target="_blank"}). That is, multiple levels may be entered (e.g., 'correct, '), the frequency of which may be adjusted relatively to each other. 

Probabilities: Proportion of each level, overall or per cell (comma-separated, e.g., \'.2, .8\'), with values between 0 and 1. Values towards 0 increase proportion of first-entered levels; values towards 1 increase last-entered levels. Please check ongoing result in the summary table, and if necessary, rearrange levels or increase the total sample size.

If any within-participant factors were created, each row in the summary table will represent a unique combination of the levels of factors, which is known as a *cell* in a factorial design. The number of cells will be displayed at the top bar. To assign each cell a particular value in a dependent variable, the user may enter multiple values separated by commas in the same fields (specifically, in *Probabilities*, *Means* or *SDs*). Note that the *Levels* fields also accept several entries, but these entries are not related to the number of cells.

For instance, with a 2x2 design (four cells), the levels of a categorical dependent variable could be entered as 'correct, incorrect, unresponded', and the probabilities as $.2, .8, .4, .7$. This would mean that the most frequent level in Cells 1 to 4, respectively, would be 'correct', 'unresponded', 'incorrect', 'unresponded'.

For a continuous variable, the means could be entered as $234, 543, 657, 123$, and the standard deviations as $10, 20, 25, 5$.

Fewer values than the number of cells could be entered in Means, SDs and Probabilities&mdash;specifically, any fraction of the number of cells (e.g., one or two values instead of four)&mdash;, but this would cancel the difference between some factor levels. 

The result from the inputs may be checked in the summary table (see [general example](#home)).

#### **Continuous**

The means could be entered as $234, 543, 657, 123$, and the standard deviations as $10, 20, 25, 5$. , as shown in the summary table below.

Please note that the means and standard deviations entered here are used in a random simulation ([`rnorm`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/Normal.html){target="_blank"}). Therefore, the final data will slightly differ, especially with smaller samples. Last, if you wish to ensure the reproducibility of the simulated data, please download the parameter list in the [last tab](#dataset), and the [final data set](#dataset).

Rounding: select number of decimal places

```{r}

numericInput('rounding', NULL, 3, min = 1)

```

### Example

In an example of a 2x2 design (four cells), one of the categorical variables could have the levels 'correct, incorrect, unresponded', and probabilities of ........

```{r}

selectInput('examples', NULL, c('None', 'Example 1', 'Example 2', 'Example 3'))

```

### Citing this app

Bernabeu, P., & Lynott, D. (2020). Experiment simulation app [Web application]. https://github.com/pablobernabeu/Experiment-simulation-app/.

### Code

This web application was developed in [R](https://www.r-project.org/about.html){target="_blank"} (R Core Team, 2020). The code is [available on Github](https://github.com/pablobernabeu/Experiment-simulation-app/blob/master/experiment-simulation.Rmd){target="_blank"}, where contributions may be made. Part of this work was influenced by Crump (2017), in particular by [Section 5.7 *Simulating data for multi-factor designs*](https://crumplab.github.io/programmingforpsych/simulating-and-analyzing-data-in-r.html#simulating-data-for-multi-factor-designs){target="_blank"}.

### References

<div style = "text-indent:-1.5em; margin-left:1.5em; background-color:#FCFCFC;">
...
To submit any questions or feedback, please post a [GitHub issue](https://github.com/pablobernabeu/Experiment-simulation-app/issues){target="_blank"} or email Pablo Bernabeu at p.bernabeu@lancaster.ac.uk.
...
factorial_structure = reactive({
  
  # Create grouping cluster
  structure = data.frame(Participant()) %>%
    ...
    # Order columns (various name versions need to be included)
    select(any_of(c(
      'BetweenGroup1', 'BetweenGroup2', 'BetweenGroup3',
      'WithinParticipant1', 'WithinParticipant2', 'WithinParticipant3',
      'Trial', 'Extra_nested'
    ...
    # Gather
    group_by(across(any_of(which(names(.) %in% 
                                   c('BetweenGroup1', 'BetweenGroup2', 'BetweenGroup3',
                                     'WithinParticipant1', 'WithinParticipant2', 'WithinParticipant3',
                                     'Trial', 'Extra_nested'))))) %>%
  ...
  if(isTruthy(input$name_BetweenGroup1) & isTruthy(input$name_BetweenGroup2) & isTruthy(input$name_BetweenGroup3)) {
    names(structure)[names(structure)=="BetweenGroup1"] = input$name_BetweenGroup1
    names(structure)[names(structure)=="BetweenGroup2"] = input$name_BetweenGroup2
    names(structure)[names(structure)=="BetweenGroup3"] = input$name_BetweenGroup3
  }
  
  if(isTruthy(input$name_BetweenGroup2) & !isTruthy(input$name_BetweenGroup3)) {
    structure = structure[, !(names(structure) == 'BetweenGroup3')]
  }
  
  if(isTruthy(input$name_BetweenGroup1) & !isTruthy(input$name_BetweenGroup2)) {
    structure = structure[, !(names(structure) %in% c('BetweenGroup3', 'BetweenGroup2'))]
  }
  
  if(!isTruthy(input$name_BetweenGroup1)) {
    structure = structure[, !(names(structure) %in% c('BetweenGroup3', 'BetweenGroup2', 'BetweenGroup1'))]
  }
  
  if(isTruthy(input$name_WithinParticipant1) & isTruthy(input$name_WithinParticipant2) & isTruthy(input$name_WithinParticipant3)) {
    names(structure)[names(structure)=="WithinParticipant1"] = input$name_WithinParticipant1
    names(structure)[names(structure)=="WithinParticipant2"] = input$name_WithinParticipant2
    names(structure)[names(structure)=="WithinParticipant3"] = input$name_WithinParticipant3
  }
  
  if(isTruthy(input$name_WithinParticipant2) & !isTruthy(input$name_WithinParticipant3)) {
    structure = structure[, !(names(structure) == 'WithinParticipant3')]
  }
  
  if(isTruthy(input$name_WithinParticipant1) & !isTruthy(input$name_WithinParticipant2)) {
    structure = structure[, !(names(structure) %in% c('WithinParticipant3', 'WithinParticipant2'))]
  }
  
  if(!isTruthy(input$name_WithinParticipant1)) {
    structure = structure[, !(names(structure) %in% c('WithinParticipant3', 'WithinParticipant2', 'WithinParticipant1'))]
  }
  
  if(isFALSE(input$include_Trial_column)) {
    structure = structure[, !(names(structure) == 'Trial')]
  }
  
  if(isFALSE(input$include_Extra_nested_column)) {
    structure = structure[, !(names(structure) == 'Extra_nested')]
  }
  ...
  if(isTRUE(input$include_Trial_column)) {
    names(structure)[names(structure)=="Trial"] = input$name_Trial_column
  }
  
  if(isTRUE(input$include_Extra_nested_column)) {
    names(structure)[names(structure)=="Extra_nested"] = input$name_Extra_nested_column
  }
  
  # Remove NA-only columns
  structure = structure[, !map_lgl(structure, ~ all(is.na(.)))]
...
Inputs {.sidebar style='padding-left: 70px; min-width: 500px; max-width: 500px; overflow: scroll; padding-right: 40px; margin-right: 140px;'}
-----------------------------------------------------------------------
...
renderUI({
  ...
  } else if(input$examples == 'Example 1') {
    tagList(
      textAreaInput(height = '45px', resize = 'vertical', 'name_Categorical_Independent1', NULL, value = 'Cat_Indiv_1'),
      textAreaInput(height = '45px', resize = 'vertical', 'levels_Categorical_Independent1', NULL, value = 'po, wer, wera'),
      textAreaInput(height = '45px', resize = 'vertical', 'probability_Categorical_Independent1', NULL, value = '.3')
    )   
  } else if(input$examples == 'Example 2') {
    tagList(
      textAreaInput(height = '45px', resize = 'vertical', 'name_Categorical_Independent1', NULL, value = 'Cat_Indiv_1'),
      textAreaInput(height = '45px', resize = 'vertical', 'levels_Categorical_Independent1', NULL, value = 'po, wer, wera'),
      textAreaInput(height = '45px', resize = 'vertical', 'probability_Categorical_Independent1', NULL, value = '.3')
    )   
  } else if(input$examples == 'Example 3') {
    tagList(
      textAreaInput(height = '45px', resize = 'vertical', 'name_Categorical_Independent1', NULL, value = 'Cat_Indiv_1'),
      textAreaInput(height = '45px', resize = 'vertical', 'levels_Categorical_Independent1', NULL, value = 'po, wer, wera'),
      textAreaInput(height = '45px', resize = 'vertical', 'probability_Categorical_Independent1', NULL, value = '.3')
...
reactive({
  if(isTruthy(input$probability_Categorical_Independent1)) {
    set.seed(input$seed)
    Categorical_Independent1 = rbinom(final_N(),
                                      # Below, count number of levels
                                      # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Independent1 = as.factor(Categorical_Independent1)
    levels(Categorical_Independent1) = levels_Categorical_Independent1
    ...
    Categorical_Independent1 %>% levels() %>% trimws() %>%
      paste0(., ': ', (table(Categorical_Independent1) / sum(table(Categorical_Independent1)) * 100) %>%
               round(digits = input$rounding), '%', collapse = '\n') %>%
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Independent2)) {
    ...
    Categorical_Independent2 = rbinom(final_N(),
                                      # Below, count number of levels
                                      # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Independent2 = as.factor(Categorical_Independent2)
    levels(Categorical_Independent2) = levels_Categorical_Independent2
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Independent3)) {
    ...
    Categorical_Independent3 = rbinom(final_N(),
                                      # Below, count number of levels
                                      # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Independent3 = as.factor(Categorical_Independent3)
    levels(Categorical_Independent3) = levels_Categorical_Independent3
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Independent4)) {
    ...
    Categorical_Independent4 = rbinom(final_N(),
                                      # Below, count number of levels
                                      # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Independent4 = as.factor(Categorical_Independent4)
    levels(Categorical_Independent4) = levels_Categorical_Independent4
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Independent5)) {
    ...
    Categorical_Independent5 = rbinom(final_N(),
                                      # Below, count number of levels
                                      # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Independent5 = as.factor(Categorical_Independent5)
    levels(Categorical_Independent5) = levels_Categorical_Independent5
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Independent6)) {
    ...
    Categorical_Independent6 = rbinom(final_N(),
                                      # Below, count number of levels
                                      # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Independent6 = as.factor(Categorical_Independent6)
    levels(Categorical_Independent6) = levels_Categorical_Independent6
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Independent7)) {
    ...
    Categorical_Independent7 = rbinom(final_N(),
                                      # Below, count number of levels
                                      # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Independent7 = as.factor(Categorical_Independent7)
    levels(Categorical_Independent7) = levels_Categorical_Independent7
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Independent8)) {
    ...
    Categorical_Independent8 = rbinom(final_N(),
                                      # Below, count number of levels
                                      # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Independent8 = as.factor(Categorical_Independent8)
    levels(Categorical_Independent8) = levels_Categorical_Independent8
...
# Count total of Categorical_Independent levels
...
Column {style='padding-left: 40px; margin-left: 230px;'}
-----------------------------------------------------------------------
...
<!-- <div style='min-width: 400px; max-width: 800px; padding-top: 16px; padding-left: 20px; padding-right: 20px; padding-bottom: 10px; background-color: #FCFCFC; font-size: 16px; line-height: 1.6;'> -->
...
<!-- The result from the inputs may be checked in the summary table. -->

<!-- [<i class="fas fa-info-circle" aria-hidden="true"></i> Further guidelines and example](#home) -->
...
<i class="fas fa-plus"></i> Nested {style="background-color: #FCFCFC; data-width: 100%; width: 900px; margin: 0 auto;"}
=======================================================================
...
checkboxInput('include_Trial_column', 'Include this nested factor', value = FALSE)
numericInput('trial_N', NULL, 1, min = 1)

textAreaInput(height = '45px', resize = 'vertical', 'name_Extra_nested_column', '', value = 'Extra_nested')
checkboxInput('include_Extra_nested_column', 'Include this nested factor', value = FALSE)
...
The factorial structure above should guide the setting of parameters in categorical and continuous dependent variables. Each row in the table corresponds to a *cell* in the current design. The overall summary including the dependent variables may be seen further below.

**Categorical: probability parameters.** To adjust the frequency of each level in each cell, multiple probability values may be entered, separated by commas, each corresponding to a cell.

**Continuous: means and standard deviations.** To adjust the means and standard deviations for each cell, multiple values may be entered, separated by commas, each corresponding to a cell.
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Dependent1)) {
    ...
    Categorical_Dependent1 = rbinom(final_N(),
                                    # Below, count number of levels
                                    # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Dependent1 %>% levels() %>% trimws() %>%
      paste0(., ': ', (table(Categorical_Dependent1) / sum(table(Categorical_Dependent1)) * 100) %>% 
               round(digits = input$rounding), '%', collapse = '\n') %>%
      str_replace_all(', ', ',') %>% str_replace_all(' ,', ',') %>% paste0(., '\n') %>% cat()
    cat('\nSee summary table for\nproportions per cell.')
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Dependent2)) {
    ...
    Categorical_Dependent2 = rbinom(final_N(),
                                    # Below, count number of levels
                                    # One unit has to be subtracted from the word count to make rbinom() accurate
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Dependent3)) {
    ...
    Categorical_Dependent3 = rbinom(final_N(),
                                    # Below, count number of levels
                                    # One unit has to be subtracted from the word count to make rbinom() accurate
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Dependent4)) {
    ...
    Categorical_Dependent4 = rbinom(final_N(),
                                    # Below, count number of levels
                                    # One unit has to be subtracted from the word count to make rbinom() accurate
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Dependent5)) {
    ...
    Categorical_Dependent5 = rbinom(final_N(),
                                    # Below, count number of levels
                                    # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Dependent5 = as.factor(Categorical_Dependent5)
    levels(Categorical_Dependent5) = levels_Categorical_Dependent5
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Dependent6)) {
    ...
    Categorical_Dependent6 = rbinom(final_N(),
                                    # Below, count number of levels
                                    # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Dependent6 = as.factor(Categorical_Dependent6)
    levels(Categorical_Dependent6) = levels_Categorical_Dependent6
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Dependent7)) {
    ...
    Categorical_Dependent7 = rbinom(final_N(),
                                    # Below, count number of levels
                                    # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Dependent7 = as.factor(Categorical_Dependent7)
    levels(Categorical_Dependent7) = levels_Categorical_Dependent7
...
reactive({
  ...
  if(isTruthy(input$probability_Categorical_Dependent8)) {
    ...
    Categorical_Dependent8 = rbinom(final_N(),
                                    # Below, count number of levels
                                    # One unit has to be subtracted from the word count to make rbinom() accurate
    ...
    Categorical_Dependent8 = as.factor(Categorical_Dependent8)
    levels(Categorical_Dependent8) = levels_Categorical_Dependent8
...
final_N = reactive({
  ...
  # If any custom ID labels were entered, multiply their number by N.
  # Else, if no custom ID labels were entered, multiply N by user-entered factor.
  if(!isTruthy(input$ID_labels)) {
    N = input$multiply_N
  } else {
    N = ngram::wordcount(input$ID_labels, sep = ',')
  }
...
Participant = reactive({
  # If Participant column is included by user
  ...
  if(isTRUE(input$include_Participant_column)) {
    ...
    # If Participant column is not included by user...
  } else {
    data.frame(Participant = factor(rep(seq(1, 1))))
  }
})
...
levels_BetweenGroup1 = reactive({
  if(isTruthy(input$BetweenGroup1_Group2)) {
...
levels_BetweenGroup2 = reactive({
  if(isTruthy(input$BetweenGroup2_Group2)) {
...
levels_BetweenGroup3 = reactive({
  if(isTruthy(input$BetweenGroup3_Group2)) {
...
Size (*N*) {style="background-color: #FCFCFC; data-width: 100%; width: 900px; margin: 0 auto;"}
=======================================================================

Column
---

### <span style='font-size:18px; color:#4D4F54;'> &nbsp; &nbsp; &nbsp; &nbsp;  Adjust sample size</span>

<div style = "padding-left: 60px; padding-right: 60px; background-color: #FEFEFE; font-size: 16px; line-height: 1.6; padding-top: 5px;">

<h3 style='color:#2C6367'> Current sample size (*N*): `r reactive(final_N())` </h3>

Adjust *N* by changing the multiplication factor below.

```{r}

textAreaInput(height = '45px', resize = 'vertical', 'name_Participant_column', 'Name of the base ID column', value = 'Participant')

numericInput('multiply_N', NULL, 10, min = 1)

checkboxInput('include_Participant_column', 'Include this column', value = TRUE)

textAreaInput(height = '45px', resize = 'vertical', 'ID_labels', 
              'Replace this column with names/categories (comma-separated). Delete back to revert.', 
              placeholder = "E.g., people's names, cities, treatments, etc.", width = '500px')

```

The sample size is interlocked with the factorial structure of the data set. That is, if any between-group factors are created, participants will be evenly drawn from the total *N*. Similarly, for any within-participant factors, repeated measures will be created by expanding the number of rows in the long-format data set ([last tab](#dataset)).
...
renderDT(
  datatable(custom_summary(), 
            extensions = c('Buttons'), class="nowrap compact cell-border", selection = 'none',
            options = list(order = list(1, 'asc'), dom = 'Bifrt', pageLength = nrow(custom_summary()), scroller = TRUE,
                           buttons = list(list(extend = 'csv', text = HTML('<i class="fas fa-download"></i>&nbsp; Download data set (.csv)'),
                                               filename = paste0('Custom summary of simulated data - retrieved on ',
                                                                 str_replace_all(format(Sys.time(), '%d-%b-%Y_%X-%Z'), ':', '-') %>%
                                                                   str_replace_all('\\ ', '-'), 
                                                                 ' -- find info on GitHub repository pablobernabeu-Experiment-simulation-app')))),
            # Fixed row names (source: https://stackoverflow.com/questions/55115183/r-dt-datatable-not-retaining-row-index-counter-column-after-selecting-new-page-w/55125758#55125758)
            callback = JS(c(
              "table.on('draw.dt', function(){",
              "  var PageInfo = table.page.info();",
...
Dataset &nbsp;<i class="fas fa-flag-checkered"></i>
=======================================================================

Column {style="position:static; min-width: 500px; max-width: 550px; min-height: 1000px;"}
-----------------------------------------------------------------------
...
parameters = reactive({
  ...
  parameters = paste0('\n\nParameters for the reproducibility of the simulated data\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~', 
                      '\nInformation and web application available at\nhttps://github.com/pablobernabeu/Experiment-simulation-app/\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~',
                      '\n\nSeed number: ', input$seed, '\n',
                      '\nNumber of decimal places: ', input$rounding, '\n')
  ...
  if(isTRUE(input$include_Trial_column)) {
    parameters = paste0(parameters, '\nExtra nested factor 1: included',
  ...
  if(isTRUE(input$include_Extra_nested_column)) {
    parameters = paste0(parameters, '\nExtra nested factor 2: included',
  ...
  parameters = paste0(parameters, '\nSample size (N) multiplication factor: ', input$multiply_N, '\n\n\n')
...
output$parameters_download_link = renderUI({
  downloadLink('parameters_download', HTML('<i class="fas fa-download"></i>&nbsp; Download to ensure reproducibility'), 
...
output$parameters_download = downloadHandler(
  filename = function() {
    paste0('Parameter list (seed number ', input$seed, ') - retrieved on ',
           str_replace_all(format(Sys.time(), '%d-%b-%Y_%X-%Z'), ':', '-') %>%
             str_replace_all('\\ ', '-'), 
           ' -- find info on GitHub repository pablobernabeu-Experiment-simulation-app.txt')
  },
...
renderDT(
  datatable(final_dataset(), 
            extensions = c('Buttons'), class="nowrap compact cell-border", selection = 'none',
            options = list(order = list(1, 'asc'), dom = 'Bifrt', pageLength = nrow(final_dataset()), scroller = TRUE,
                           buttons = list(list(extend = 'csv', text = HTML('<i class="fas fa-download"></i>&nbsp; Download data set (.csv)'),
